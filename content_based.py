...
def fill_ratings(rating_matrix):

    users_data = load_from_csv(os.path.join(prefix, 'data_user.csv'), delimiter=';')
    users_data = pd.get_dummies(users_data,columns=['occupation', 'gender'])
    users_data = users_data.values.squeeze()
    cos_sim = np.array(cosine_similarity(users_data,users_data))
    rating_matrix = rating_matrix.toarray()
    rating_matrix = np.column_stack(rating_matrix)
    print(np.shape(rating_matrix))

    for user_id in range(np.shape(rating_matrix)[0]):
        close_users = genre_recommendations(cos_sim[user_id])
        small_rating_matrix = [i for id,i in enumerate(rating_matrix) if id in close_users]
        small_rating_matrix[small_rating_matrix == 0] = np.nan
        print(small_rating_matrix)
        break       
...
def genre_recommendations(cos_sim):
    sim_scores = list(enumerate(cos_sim))
    sim_scores = sorted(sim_scores, key=lambda x: x[1], reverse=True)
    sim_scores = [i for i in sim_scores if i[1] > 0.9]
...
if __name__ == '__main__':
    ...
    training_user_movie_pairs = load_from_csv(os.path.join(prefix,
                                                           'data_train.csv')).values.squeeze()
    training_labels = load_from_csv(os.path.join(prefix, 'output_train.csv')).values.squeeze()
